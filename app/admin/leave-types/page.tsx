...
export default async function LeaveTypesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Table>
        <thead className="bg-surface text-left text-xs uppercase text-muted"><tr><th className="px-4 py-3">Name</th><th className="px-4 py-3">Entitlement</th><th className="px-4 py-3">Qualifies after</th><th className="px-4 py-3">Document</th><th className="px-4 py-3">Approval</th><th className="px-4 py-3">Paid</th><th className="px-4 py-3">Status</th></tr></thead>
        <tbody className="divide-y divide-line">{types.map((type) => <tr key={type.id}><td className="px-4 py-3 font-medium">{type.name}</td><td className="px-4 py-3">{type.annualEntitlementDays}</td><td className="px-4 py-3">{type.eligibilityMonths === 12 ? "1 year" : `${type.eligibilityMonths} month${type.eligibilityMonths === 1 ? "" : "s"}`}</td><td className="px-4 py-3">{type.requiresDocument ? "Yes" : "No"}</td><td className="px-4 py-3">{type.requiresApproval ? "Yes" : "No"}</td><td className="px-4 py-3">{type.isPaid ? "Yes" : "No"}</td><td className="px-4 py-3"><StatusBadge value={type.active ? "ACTIVE" : "INACTIVE"} /></td></tr>)}</tbody>
      </Table>
    </div>
  );
}
