  ...
  return (
    ...
    >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {conversationsQuery.isLoading ? <div className="space-y-1 p-2">{[1, 2, 3].map((item) => <div key={item} className="h-[76px] animate-pulse rounded-xl bg-surface" />)}</div> : visibleConversations.map((conversation) => {
            ...
            return <button key={conversation.id} type="button" onClick={() => selectConversation(conversation.id)} className={`flex w-full gap-3 border-b border-line/60 px-3.5 py-3 text-left transition sm:px-4 ${conversation.id === selectedConversationId ? "bg-brandSoft/80" : "hover:bg-surface/80"}`}>
              <span className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-brand text-sm font-bold text-white">{conversation.type === "GROUP" ? <Users className="h-5 w-5" aria-hidden /> : initials(conversation.title)}{peerOnline ? <span className="absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-white bg-emerald-500 dark:border-panel" aria-label="Online" /> : null}</span>
      ...
      <div className={`${selectedConversationId ? "flex" : "hidden lg:flex"} relative min-h-0 flex-col bg-white`}>
        ...
        ) : <div className="flex flex-1 flex-col items-center justify-center bg-surface/50 p-8 text-center"><span className="flex h-14 w-14 items-center justify-center rounded-xl border border-line bg-white text-brand dark:bg-panel"><MessageSquarePlus className="h-6 w-6" aria-hidden /></span><h2 className="mt-4 font-semibold text-ink">No conversation selected</h2><p className="mt-1 max-w-sm text-sm text-muted">Select a conversation or start a new chat.</p></div>}
      </div>
    </section>
        ...
        <SheetContent title="Conversation details" className="w-[min(28rem,calc(100vw-1rem))]">
          <div className="space-y-5">
            <div className="flex flex-col items-center rounded-xl border border-line bg-surface p-5 text-center">
              <span className="flex h-16 w-16 items-center justify-center rounded-full bg-brand text-lg font-bold text-white">{selectedConversation.type === "GROUP" ? <Users className="h-7 w-7" aria-hidden /> : initials(selectedConversation.title)}</span>
