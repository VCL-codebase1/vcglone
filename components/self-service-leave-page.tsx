...
export async function SelfServiceLeavePage({ applyHref = "/employee/leave/apply" }: { applyHref?: string }) {
  ...
  const [leaveTypes, balances, requests] = await Promise.all([
    ...
    prisma.leaveBalance.findMany({ where: { employeeId: user.id, year }, include: { leaveType: true } }),
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Leave" description={`View your ${year} leave slots, taken days, available balance, and request status.`} action={<LinkButton href={applyHref}>Apply for leave</LinkButton>} />
